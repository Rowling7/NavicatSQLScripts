...
HA0102,
	...
	[COL39] = CASE WHEN LEFT(K0101,1) IN ('G','H') THEN LTRIM(RTRIM(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(K3990,'340000',''),'000000',''),'D001',''),'D002',''),'D003',''))) ELSE '' END,		--国道调整前路线编号
	[COL40] = HK0138,		--是否按干线公路管理接养
	[COL41] = CASE WHEN CHARINDEX('D',ISNULL(K0124,'')) <> 0 THEN '重复主线为长链' ELSE CASE WHEN LEFT(K0101,1) IN ('X','Y','Z','C') THEN LTRIM(RTRIM(CAST(K0199 AS VARCHAR(1000)))) ELSE '' END END	--备注
FROM K01 
WHERE	
	LEFT(K0101,1) IN ('X','Y','Z','C') AND a0102='3421524'
