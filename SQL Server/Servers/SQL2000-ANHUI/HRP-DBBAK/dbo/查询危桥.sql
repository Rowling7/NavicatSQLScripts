/*不显示零*/
SELECT
rtrim(dz.objjc),rtrim(dz2.objjc),
...
(
	...
	WHERE A0102 LIKE '34%' AND LEFT(K0101,1) IN ('X','Y','Z','C','J','N')
) B ON A.KWYID=B.KWYID
left join dz0101 dz on left(a0103,4)=rtrim(dz.objname)  
left join dz0101 dz2 on left(a0103,6)=rtrim(dz2.objname)  
WHERE LEFT(K0101,1) IN ('X','Y','Z','C','J','N') AND A0102 LIKE '34%' AND HK6028  IN ('四类','五类')
