...
--在2000的数据库中进行
--1.2000库附加NCGL标准库
EXEC sp_attach_db @dbname = N'NCGL3420230130', 
@filename1 = N'E:\6078曹勇嵩\1.工作文件\2.安徽工作文件\2.2023年报报部工作\34_安徽2023年报规划司文件\NCGL342023_0130.MDF'
...
SELECT 'dbo.T_NCGLAFSS',count(*) from dbo.T_NCGLAFSS
union
SELECT 'dbo.T_NCGLJCSS ',count(*) from dbo.T_NCGLJCSS 
union
SELECT 'dbo.T_xz ',count(*) from dbo.T_xz 
union 
SELECT 'dbo.T_jzc',count(*) from dbo.T_jzc
union 
SELECT 'dbo.T_ZRC',count(*) from dbo.T_ZRC
union 
SELECT 'dbo.T_ZRCTYXTDLX',count(*) from dbo.T_ZRCTYXTDLX
union 
SELECT 'dbo.T_ld',count(*) from dbo.T_ld
union
SELECT 'dbo.T_ql',count(*) from dbo.T_ql
union
SELECT 'dbo.T_sd',count(*) from dbo.T_sd
union
SELECT 'dbo.T_dk',count(*) from dbo.T_dk
...
(
	...
	WHERE LEFT(A.K0101,1) IN ('X','Y','C') AND (A.K0124 is null or A.K0124 = '') and isnull(A.K0123,'2') = '2'
...
SELECT 
	...
	CASE WHEN LEFT(K0124,1) IN ('X','Y','C') THEN NULL ELSE K0162 END,	--重复路段起点桩号
	CASE WHEN LEFT(K0124,1) IN ('X','Y','C') THEN NULL ELSE K0163 END,	--重复路段止点桩号
...
WHERE LEFT(K0101,1) NOT IN ('G','S','H','T','W','Z') 
...
if exists(select * from tempdb..sysobjects where id=object_id('tempdb..#CFLD')) drop table #CFLD 
...
WHERE LEFT(A.SGYLDBM,1) IN ('X','Y','C')
...
WHERE LEN(A.K0101)=10 AND LEFT(A.K0101,1) NOT IN ('G','S','H','T','W','Z') 
...
WHERE LEFT(A.K0101,1) NOT IN ('G','S','H','T','W','Z')
...
--在2005的数据库中进行
--4.2005附加第三步分离的数据库
CREATE DATABASE NCGL342023   ON (FILENAME = 'E:\NCGL\202401281608\NCGL342023_0128.MDF') FOR ATTACH;
